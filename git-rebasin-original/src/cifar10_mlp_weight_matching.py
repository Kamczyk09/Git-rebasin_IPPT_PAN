...
def main():
  ...
  with wandb.init(
      ...
      entity="skainswo",
      tags=["cifar10", "mlp", "weight-matching"],
      job_type="analysis",
  ) as wandb_run:
    config = wandb.config
    ...
    fig = plot_interp_loss(config.load_epoch, lambdas, train_loss_interp_naive,
                           ...
                           test_loss_interp_clever)
    plt.savefig(f"cifar10_mlp_weight_matching_interp_loss_epoch{config.load_epoch}.png", dpi=300)
    ...
    fig = plot_interp_acc(config.load_epoch, lambdas, train_acc_interp_naive, test_acc_interp_naive,
                          train_acc_interp_clever, test_acc_interp_clever)
    plt.savefig(f"cifar10_mlp_weight_matching_interp_accuracy_epoch{config.load_epoch}.png",
                dpi=300)
